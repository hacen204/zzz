...
def KR(C,
    ...
    if start_curve is None:
        start_curve = 1
        start_curve_masked = np.ones(Nt)
    else : 
        start_curve_masked = start_curve[arr_msk_col]

    # get coefficient vector beta. shape of beta is (nt,)
    beta=(C_masked.T@CKC_inv)@(B-C_masked@start_curve_masked)
    # get discount vector with length Nmax
    g_solved=  start_curve + K.take(arr_msk_col,axis=1)@beta
